<template>
    <div>
        <v-layout align-center justify-start row class="pr-2 pl-2">
            <!--
      =====================================================================================
        DETAIL TITLE
      =====================================================================================
      -->
            <v-flex xs4 sm3 md2>
                <div class="text-dark font-weight-bold">{{ detail.title }}</div>
            </v-flex>
            <!--
      =====================================================================================
        DETAIL INFO
      ...
      -->
            <v-flex v-if="!detail.txInput" xs7 sm8 md9 pr-0>
                <div v-if="detail.tokenTransfers">
                    <span v-for="(item, index) in detail.tokenTransfers" :key="index" class="mb-2" style="display: block">
                        <b class="mr-1">From</b>
                        <a class="hash-tag text-truncate" :href="`/address/${item.from}`">
                            <app-transform-hash :hash="item.from | toChecksum" />
                        </a>
                        <b class="mx-1">To</b>
                        <a class="hash-tag text-truncate" :href="`/address/${item.to}`">
                            <app-transform-hash :hash="item.to | toChecksum" />
                        </a>
                        <b class="mx-1">For</b>
                        <span
                            >{{ item.value.value }} <b class="text-uppercase">{{ item.value.unit }}</b>
                        </span>
                    </span>
                </div>
                <div v-if="isLoading">
                ...
                <div v-else>
                    <div v-if="!isMono">
                        ...
                        <div v-else class="text-dark text-truncate detail-container">
