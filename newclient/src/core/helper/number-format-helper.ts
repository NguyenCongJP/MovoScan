...
import { EthValue, CmcValue } from '@app/core/models'
...
export class NumberFormatHelper {
    ...
    public static formatVariableUnitCmcValue(value: BigNumber): FormattedNumber {
        /**
         * Case I: value === 0
         * Return: "0 ETH"
         */
        if (value.isZero()) {
            return { value: '0', unit: FormattedNumberUnit.CMC }
        } else if (value.isLessThan(TenThousand)) {
            /**
             * Case II: value < 10,000 wei
             * Return: small values in WEI (no conversion) and tooltip with ETH value
            ...
            return {
                value: value.toFormat(),
                unit: FormattedNumberUnit.WEI,
                tooltipText: `${new CmcValue(value).toCmcBN().toFixed()}`
            }
        } else if (value.isLessThan(HundredBillion)) {
            /**
             * Case III: value < 100 Billion Wei OR value < 100 Gwei
             * Return: Gwei value, using Group II
             */
            const gweiBN = new CmcValue(value).toGweiBN()
            return {
                value: this.formatFloatingPointValue(gweiBN).value,
                unit: FormattedNumberUnit.GWEI,
                tooltipText: `${new CmcValue(value).toCmcBN().toFixed()}`
        ...
        const ethBN = new CmcValue(value).toCmcBN()
        const unit = FormattedNumberUnit.CMC
        const dps = ethBN.decimalPlaces()
    ...
    public static formatNonVariableCmcValue(value: BigNumber): FormattedNumber {
        /**
         * Case I: value === 0
         * Return: "0 ETH"
         */
        ...
        const ethBN = new CmcValue(value).toCmcBN()
        return { ...this.formatFloatingPointValue(ethBN), unit: FormattedNumberUnit.CMC }
    }
