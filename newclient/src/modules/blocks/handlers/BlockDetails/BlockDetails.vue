...
export default class BlockDetails extends Mixins(NumberFormatMixin, NewBlockSubscription) {
    ...
    */
        ...
        if (this.loading) {
            ...
            details = [
                ...
                },
                {
                    ...
                    tooltip: `${this.rewards.tooltipText} ${this.$i18n.t('common.movo')}` || undefined
                },
                {
                    title: this.$i18n.tc('tx.fee', 2),
                    detail: `${this.transactionFees.value} ${this.transactionFees.unit}`,
                    tooltip: this.transactionFees.tooltipText ? `${this.transactionFees.tooltipText} ${this.$i18n.t('common.movo')}` : undefined
                },

                {
                    title: this.$i18n.t('uncle.reward'),
                    detail: `${this.uncleRewards.value} ${this.uncleRewards.unit}`,
                    tooltip: this.uncleRewards.tooltipText ? `${this.uncleRewards.tooltipText} ${this.$i18n.t('common.movo')}` : undefined
                },
                {
                    title: this.$i18n.tc('tx.name', 2),
    ...
    get rewards(): FormattedNumber {
        return this.formatVariableUnitCmcValue(new BN(this.block.summary.rewards.total))
    }
    get uncleRewards(): FormattedNumber {
        return this.formatVariableUnitCmcValue(new BN(this.block.summary.rewards.uncles))
    }
    get transactionFees(): FormattedNumber {
        return this.formatVariableUnitCmcValue(new BN(this.block.summary.rewards.txFees))
    }
