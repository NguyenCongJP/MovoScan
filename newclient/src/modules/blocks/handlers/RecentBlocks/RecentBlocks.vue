...
export default class RecentBlocks extends Vue {
    ...
    */
    ...
    @Prop({ type: Array, default: () => [] }) topicBlock!: any[]
    ...
    */
    ...
    get blockAndTopic(): any {
        const topic: any[] = this.topicBlock.map(x => {
            let y = {
                miner: `${x.hash}`,
                ...
                timestamp: Math.floor(parseInt(x.id)/1000),
                txCount: 0,
                txFail: 0,
                _typename: 'TopicBlock'
            }
            return y
        })
        const getBlocksArrayByNumber = this.getBlocksArrayByNumber
        const bt = topic.concat(getBlocksArrayByNumber)
        return bt.sort((a,b) => b.timestamp - a.timestamp)
    }
    ...
    */
    ...
    //     }
    // }
    @Watch('blockAndTopic', { deep: true })
    onGetBlocksArrayByNumberChanged(val: any[], oldVal: any[]) {
        if (val != oldVal) {
            this.$set(this.indexedBlocks, this.index, val)
        }
    }
