<template>
    ...
    =====================================================================================
    -->
    <v-layout row wrap justify-start class="py-0 mb-4">
...
export default class TxDetails extends Mixins(NumberFormatMixin) {
  ...
  */
    ...
    get txDetails(): Detail[] {
        ...
        if (this.isLoading) {
            ...
            details = [
                ...
                {
                    title: isContractCreation ? this.$t('contract.creation').toString() : this.$i18n.t('tx.to').toString(),
            ...
            if (this.transaction.tokenTransfers) {
                const tokenTransfers = this.transaction.tokenTransfers.map(item => {
                    // console.log(new BN('0x' + item.value).dividedBy(new BN(Math.pow(10, 18))).toString())
                    let valueToken = item.value
                    if (item.decimals !== null && item.value !== null) {
                        let valueInHex = item.value.toString()
                        if (item.value.slice(0, 2) != '0x') {
                            valueInHex = '0x' + item.value
                        }

                        valueToken = new BN(valueInHex).dividedBy(new BN(Math.pow(10, Number(item.decimals)))).toString()
                    }
                    return {
                        from: item.from,
                        to: item.to,
                ...
                details.splice(3, 0, {
                    title: this.$i18n.t('tx.token_tranfer'),
                    tokenTransfers
                })
            }
    ...
    get txAmount(): FormattedNumber {
        return this.formatVariableUnitCmcValue(new BN(this.transaction.value))
    }
    /**
    ...
    get txFee(): FormattedNumber {
        if (this.transaction && this.transaction.gasUsed) {
            ...
            return this.formatVariableUnitCmcValue(fee)
        }
