<template>
    <div>
        <div v-if="pageType == 'home'" class="card h-100">
            <div class="card-header">
                <h2 class="card-header-title">{{ getTitle }}</h2>
            </div>
            <div class="js-scrollbar card-body" style="height: 400px; overflow-y: auto;">
                <table-txs
                    :max-items="maxItems"
                    :index="index"
                    :is-loading="loading"
...
export default class BlockTxs extends Vue {
    ...
    */
    ...
    @Prop({ type: Array, default: () => [] }) topicTransaction!: any[]
    ...
    */
    ...
    get transactions(): (TypeTransfers | null)[] | [] {
        ...
        if (this.isBlock && this.getBlockTransfers && this.getBlockTransfers.transfers !== null) {
            response = this.getBlockTransfers.transfers
        }
        if (!this.isBlock && this.getAllEthTransfers && this.getAllEthTransfers.transfers !== null) {
            response = this.getAllEthTransfers.transfers
        }
        // console.log('response',response)
        // console.log('topicTransaction',this.topicTransaction)
        const topicTransaction = this.topicTransaction.map(x => {
            let y = {
                transfer: {
                    block: parseInt(x.block),
                    from: x.from,
                    status: x.status,
                ...
                value: x.value,
                _typename: 'TopicTransaction'
            }
            return y
        })
        response = response.concat(topicTransaction)
        return response.sort((a,b) => b.transfer.timestamp - a.transfer.timestamp)
    }
