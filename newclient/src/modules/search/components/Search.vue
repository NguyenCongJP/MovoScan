<template>
    <div class="searchInput" style="display: flex; flex-wrap: wrap">
        <v-select
            v-model="selectVal"
            style="width: 15%"
            :flat="true"
        ...
        <v-combobox
            ...
            @change="onSelect"
        ></v-combobox>
        <v-flex style="width: 15%" class="search-button-container">
            <v-btn depressed color="secondary" class="search-button m-0 text-capitalize ml-0" @click="onSearch">
                <i :class="{ 'fas fa-circle-notch fa-spin': loadingSearch }"></i> <span class="ml-1">{{ $t('search.name') }}</span>
            </v-btn>
        </v-flex>
    </div>
...
.searchInput {
    ...
    .search-select.v-select .v-input__slot {
